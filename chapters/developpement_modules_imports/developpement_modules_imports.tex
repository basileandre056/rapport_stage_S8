...
Le module interrogeait alors \url{https://quadrige-core.ifremer.fr/api/extraction/doc?doc=standard&lang=fr&name=result&type=standard}

pour récupérer les observations correspondantes \href{https://quadrige.ifremer.fr/support/Mes-donnees/J-extrais-mes-donnees/J-interroge-l-api-pour-extraire-mes-donnees/Je-regarde-des-videos-de-demo-sur-l-\gls{api}}{Tutoriels vidéo}.
...
spécifiques de l’\gls{api} GraphQL mise à disposition par l’Ifremer. L’objectif principal était de 
...
\begin{itemize}
  \item l’URL de l’\gls{api} GraphQL \gls{quadrige} ;
...
territoire donné. Cette extraction repose sur une requête GraphQL spécifique fournie par l’\gls{api} 
...
\begin{itemize}
  \item erreurs de communication avec l’\gls{api} GraphQL ;
...
\texttt{\gls{api2gn}} a été conçu pour permettre l’intégration directe et automatisée de données issues de sources externes, 
exposées via des services web (\gls{api} REST, flux JSON, services WFS).

Son principe repose sur une séparation claire entre :
\begin{itemize}
\item le \textbf{cœur du moteur d’import}, chargé de l’orchestration des appels \gls{api}, de la gestion des erreurs, de 
...
s’appuyant sur un socle commun fourni par \texttt{\gls{api2gn}} : gestion des géométries, résolution des nomenclatures, validation du 
mapping, historisation des imports et intégration dans les mécanismes de sécurité de GeoNature.

Dans ce cadre, le module \texttt{\gls{api2gn}} apparaît comme un outil particulièrement adapté à l’intégration de données issues de 
...
se distinguent fortement des bases institutionnelles comme \gls{quadrige}.
...
Alors que l’import \gls{quadrige} vise des données institutionnelles, fortement structurées et produites dans un 
...
celle retenue pour \gls{quadrige} : une automatisation poussée, compensée par des mécanismes renforcés de validation et de traçabilité.
...
Sur le plan technique, le parser Pl@ntNet repose sur l’architecture standard des parseurs \texttt{\gls{api2gn}}.
...
\item construction des requêtes vers l’\gls{api} Pl@ntNet ;
...
Les paramètres d’appel à l’\gls{api}, les filtres appliqués, le mapping des champs et les règles de validation sont intégralement 
...
\begin{itemize}
\item les paramètres d’accès à l’\gls{api} Pl@ntNet (URL, clé \gls{api}) ;
...
Pour chaque observation, le nom scientifique fourni par l’\gls{api} Pl@ntNet est d’abord normalisé afin de supprimer les mentions infra-spécifiques ou les annotations non standard. Le parser tente ensuite de résoudre le \texttt{cd\_nom}
 selon un ordre hiérarchique précis :
\begin{enumerate}
...
\item vérification de l’existence du \texttt{cd\_nom}
 obtenu dans la base locale.
\end{enumerate}
...
Dans ce mode, toute observation pour laquelle aucun \texttt{cd\_nom}
 valide ne peut être résolu est rejetée. Ces rejets sont explicitement journalisés, permettant :
\begin{itemize}
...
Le parser Pl@ntNet s’intègre pleinement aux mécanismes de traçabilité fournis par \texttt{\gls{api2gn}}.
...
\begin{itemize}
\item la pertinence de l’architecture \texttt{\gls{api2gn}} pour l’intégration de sources hétérogènes ;

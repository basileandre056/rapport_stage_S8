...
\begin{itemize}
  \item un CSV brut issu directement de la réponse de l’\gls{api} \gls{quadrige}, contenant l’ensemble des instances des programmes dont au moins une 
        occurrence est localisée sur le territoire ciblé ;
  \item un CSV filtré géographiquement, obtenu par un traitement a posteriori à l’aide de la bibliothèque pandas, permettant d’exclure 
...
Seuls les champs nécessaires à l’intégration dans GeoNature étaient retenus,
conformément aux exigences du modèle \gls{sinp}.
Une transformation était appliquée pour obtenir une
...
Chaque opération d’import était consignée dans un
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.9\textwidth]{croped_images/data_flow_schematics/extraction_programs_flow.png}
    \caption{Schéma fonctionnel des échanges de données entre GeoNature et le système Quadrige lors de l'extraction des programmes}
    \label{fig:data-flow-program-extraction}
\end{figure}

L’intérêt principal de ce schéma réside dans la manière dont sont traités
les résultats retournés par l’API Quadrige lors de l’extraction des programmes.
En effet, la requête GraphQL adressée à l’API permet d’identifier les programmes
possédant au moins une instance associée à la \textit{monitoring location}
sélectionnée par l’utilisateur. Toutefois, la réponse fournie par Quadrige
prend la forme d’un fichier CSV contenant l’ensemble des instances de ces programmes,
y compris celles localisées en dehors du périmètre géographique ciblé.

Un traitement complémentaire est donc nécessaire côté backend GeoNature.
Le CSV brut généré par l’API est d’abord conservé,
puis filtré a posteriori afin de ne retenir que les instances effectivement
rattachées à la \textit{monitoring location} sélectionnée.
Ce filtrage repose notamment sur l’analyse des codes de localisation,
permettant d’exclure les stations hors territoire d’intérêt.


À l’issue de ce traitement, un CSV filtré est produit.
Il ne conserve que les informations pertinentes pour la suite du processus,
telles que le code du programme, son état, les dates associées,
l’organisme responsable et les métadonnées nécessaires à la sélection
des programmes dans l’interface GeoNature.
Ce fichier constitue la base fonctionnelle utilisée par le frontend
pour l’affichage, la recherche et la sélection des programmes à extraire.
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.9\textwidth]{croped_images/data_flow_schematics/extraction_data_flow.png}
    \caption{Schéma fonctionnel des échanges de données entre GeoNature et le système Quadrige lors de l'extraction des données des programmes}
    \label{fig:data-flow-data-extraction}
\end{figure}

Ce schéma met en évidence le fonctionnement spécifique
de la phase d’extraction des données associées aux programmes Quadrige.

Lors de cette phase,
la \textit{monitoring location} n’est volontairement pas fournie
dans les filtres transmis par l’utilisateur.
Elle est automatiquement reprise
à partir des programmes précédemment extraits
et filtrés lors de la phase d’extraction des programmes.
Ce choix permet de garantir
une cohérence stricte
entre le périmètre géographique utilisé
pour identifier les programmes
et celui appliqué lors de l’extraction des observations.

L’extraction des données est alors exécutée
programme par programme,
à partir de la liste sélectionnée par l’utilisateur
dans l’interface GeoNature.
Cette liste provient exclusivement
du CSV filtré produit lors de l’extraction des programmes.
Seuls les programmes effectivement rattachés
au territoire d’intérêt
peuvent ainsi être traités.

Ce chaînage explicite entre les deux phases
permet d’éviter les incohérences de paramétrage,
notamment liées à une erreur de localisation.
Il sécurise l’exécution des extractions
en s’appuyant sur un périmètre déjà validé,
plutôt que sur une saisie manuelle répétée des filtres.


Plus largement,
ce schéma met en évidence
la séparation volontaire
entre la phase d’extraction des programmes
et celle de l’extraction des données associées,
qui constitue un choix technique structurant du module.


La phase d’extraction des programmes
permet d’identifier, filtrer et qualifier
les programmes disponibles sur un territoire donné,
aboutissant à une liste contrôlée
servant de base à l’ensemble des opérations ultérieures.

La phase d’extraction des données
s’appuie exclusivement sur cette liste
et intervient dans un second temps,
une fois les programmes visualisés et sélectionnés.
Cette organisation permet à l’utilisateur
de conserver une maîtrise fine
sur le périmètre fonctionnel des extractions
avant de lancer des traitements potentiellement longs.
...
La liste centrale prend la forme d’un tableau synthétique,
dans lequel chaque ligne correspond à un programme Quadrige distinct.
Pour chacun d’eux sont affichés son identifiant,
son libellé descriptif,
sa date de début,
son état d’activité
ainsi que les organismes ou responsables associés.
Ces informations constituent les métadonnées minimales
nécessaires à une sélection éclairée des programmes à traiter.


Compte tenu du nombre potentiellement élevé de programmes retournés,
un mécanisme de recherche dynamique par mots-clés
est intégré directement à l’interface.
La barre de recherche permet de filtrer en temps réel
la liste affichée
en s’appuyant sur les métadonnées textuelles des programmes,
facilitant ainsi l’identification rapide
d’un programme particulier
ou d’un ensemble thématique cohérent.
...
Ce schéma illustre le fonctionnement global du parser Pl@ntNet

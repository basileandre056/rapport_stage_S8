...
un client Python permettant de tester l’envoi de requêtes GraphQL auprès de l’API Quadrige, de récupérer les données 
...
Les critères sont sauvegardés dans le fichier \texttt{memory/last\_filter.json}.  
...
Un bouton «~Extraire les programmes~» déclenche la fonction d’extraction, qui envoie la requête GraphQL à Quadrige 
...
Cependant, ce fichier inclut aussi les instances situées hors de La Réunion. J’ai donc ajouté un filtrage supplémentaire pour 
ne conserver que les lignes correspondant à la localisation souhaitée, ainsi que les champs pertinents affichés dans 
le composant \texttt{program-list}.
...
l’authentification auprès de l’API Quadrige, l’interrogation des programmes disponibles et la structuration des données 
...
Le développement du frontend a permis de mettre en place une première interface d’exploration des programmes Quadrige 
...
En parallèle du développement principal consacré à Quadrige, j’ai également initié un travail structurant autour de l’intégration des données \gls{plantnet} dans GeoNature. 
L’objectif était double~: d'une part valider la possibilité technique d’interroger l’API Pl@ntNet depuis un service Python, et d'autre part poser les bases d’un parseur Darwin Core utilisable par un module GeoNature, selon une logique similaire à celle mise en place pour Quadrige.

Pour cela, j’ai développé un client Python générique capable de~:
\begin{itemize}
    \item envoyer des requêtes POST vers l’endpoint \texttt{dwc/occurrence/search} de l’API Pl@ntNet~;
...
\[
\text{API} \;\rightarrow\; \text{JSON brut} \;\rightarrow\; \text{CSV Darwin Core normalisé}
\]

entièrement automatisé et adaptable à d’autres territoires ou taxons via un simple fichier de configuration.

Ce client constitue aujourd’hui la base technique du parser Pl@ntNet destiné au module \texttt{api2gn}, dont l’intégration a nécessité plusieurs ajustements dans les migrations SQL et dans la structure interne du module.
